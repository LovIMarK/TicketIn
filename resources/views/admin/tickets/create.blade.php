{{--
  ...
  Purpose:
  - Display a form for creating a new ticket with title, user assignment, initial message, and optional attachment.

  Expects (from controller/route):
  - route('tickets.store') available
  - $users : Collection|array of selectable users, compatible with <x-layouts.form-select :options="$users" />
...
<x-layouts.admin>
    <div class="max-w-3xl mx-auto p-6">
        ...
        <form action="{{ route('tickets.store') }}" method="POST" enctype="multipart/form-data" class="bg-white border border-2b1c50 rounded-xl shadow p-6 space-y-6">
            ...
            <!-- user selection -->
            <x-layouts.form-select id="user_id" value="" name="user_id" label="Assign User" :options="$users" required />
