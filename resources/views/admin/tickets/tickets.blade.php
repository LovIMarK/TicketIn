        ...
        <section class="w-1/3 border-r border-gray-400 overflow-y-auto p-4">
            <div class="mb-4">
                ...
                <select id="filter" class="w-full p-2 border rounded">
                    <option disabled {{ is_null($currentFilter) ? 'selected' : '' }}>Filter by...</option>
                    <option value="priority" {{ $currentFilter === 'priority' ? 'selected' : '' }}>Priority</option>
                    <option value="status" {{ $currentFilter === 'status' ? 'selected' : '' }}>Status</option>
                    <option value="date" {{ $currentFilter === 'date' ? 'selected' : '' }}>Creation Date</option>
                </select>
            </div>
            ...
            <ul id="ticketList">
                @forelse($tickets as $ticket)
                    <li class="ticket-item relative bg-[#2b1c50] text-[#f0f1ff] rounded shadow mb-3 p-3 cursor-pointer hover:bg-indigo-800/30 transition-colors"
                        data-url="{{ route('admin.tickets.summary', $ticket) }}">
